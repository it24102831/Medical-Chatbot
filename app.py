...
def chat():
    ...
    if not msg:
        return jsoniffrom flask import Flask, render_template, jsonify, request
from langchain_pinecone import PineconeVectorStore
from langchain_core.globals import set_llm_cache
from langchain_core.exceptions import OutputParserException
from langchain_openai import ChatOpenAI
...
if __name__ == '__main__':
    # For local development, debug mode can be enabled.
    # For production (like Cloud Run), Gunicorn will be used and debug will be off.
    port = int(os.environ.get("PORT", 8080))
    app.run(host="0.0.0.0", port=port, debug=False)y({"error": "Please enter a question."}), 400
